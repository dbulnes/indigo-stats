...
The daily local SQLite snapshot and its integrity check remain the source of every remote copy. A snapshot is written to a private partial file and atomically published only after its SQLite integrity check succeeds. An independent hourly job reconciles retained local snapshots that are missing from the selected destination, newest first. Local publication and remote reconciliation are serialized so retention cannot remove a snapshot during transfer. The service uploads the database first and a SHA-256 manifest last; a new copy is complete only after the provider-stored bytes and manifest verify. Remote failures do not make the local backup fail. One destination can be active at a time, and switching destinations uses a separate transfer-history fingerprint.

Snapshots include the private settings table and raw sensor metadata. Indigo Stats does **not** add client-side encryption in this release. Use provider/filesystem access controls and encryption at rest, keep destinations private, and protect all credentials. SHA-256 detects corruption; it does not authenticate a destination against an administrator or provider that can replace both the database and its manifest. The System API and UI expose only configured/readiness flags, never bucket names, endpoints, paths, OAuth callback URLs, folder IDs, tokens, or remote object references.

Indigo Stats has no application login. Every client with network access is effectively an administrator and can select a different destination or Google account. Keep the port restricted to a trusted LAN/tailnet or place it behind an authenticated private reverse proxy. Browser mutations require a same-origin UI header and cross-site OAuth starts are rejected to reduce CSRF risk; those checks are not a substitute for an authentication boundary against another trusted-network client.
...
Choose S3 in System and enter the bucket, optional prefix/region, optional custom endpoint, and encryption mode. Custom endpoints must use HTTPS and cannot contain credentials, query parameters, fragments, or path traversal. Supply credentials through `BACKUP_S3_ACCESS_KEY_ID`, `BACKUP_S3_SECRET_ACCESS_KEY`, and optional `BACKUP_S3_SESSION_TOKEN`, or the corresponding `_FILE` variables. For SSE-KMS, optionally supply `BACKUP_S3_KMS_KEY_ID` (or `_FILE`). Grant only bucket listing and get/put/delete access beneath the selected prefix. Provider-default encryption, SSE-S3, and SSE-KMS are supported. Indigo Stats requests an S3 SHA-256, stores the checksum as metadata, then streams the new database object back and hashes it before publishing and reading back the manifest. This deliberate read-back doubles transfer bandwidth for a newly uploaded snapshot.
...
Create a Google OAuth web client and supply `BACKUP_GOOGLE_CLIENT_ID`, `BACKUP_GOOGLE_CLIENT_SECRET`, and `BACKUP_GOOGLE_CALLBACK_URI` (or `_FILE`). The callback must exactly match the registered stable private HTTPS URL; only the exact `localhost`, `127.0.0.1`, or `::1` hostnames may use HTTP. Select Google Drive, save, then use **Link Google Drive** (which opens a dedicated popup modal for authorization and automatically refreshes on completion). Authorization requests offline access, PKCE, bounded expiring CSRF state, and only the `drive.file` scope. Indigo Stats creates a visible app-owned “Indigo Stats Backups” folder and rediscovers it using private app properties. It verifies Google Drive's server-computed SHA-256 and size for both objects before removing a superseded pair. The refresh token is mode `0600` at `/data/secrets/google-drive-token.json`, outside SQLite and its snapshots. A revoked grant produces a relink-required error; temporary authentication transport failures remain retryable. Disaster recovery requires the same OAuth project followed by relinking.
...
Save configuration before using **Test connection** or **Back up now**; the buttons always operate on the persisted destination, not unsaved form values. A second manual request or a configuration change returns conflict while a run is active. Mount/provider outages and incomplete pruning are reported with sanitized errors and retried hourly. Pruning starts only after verified copies exist and deletes only Indigo Stats snapshot/manifest names; unrelated remote files are untouched.
